...
import { Recipe, Step, Component } from "@/recipe.ts";
...
describe("Recipe.parse", () => {
    test("should parse a recipe with full metadata, notes, and multiple components", () => {
        ...
        expect(recipe.components[0]?.steps[0]?.ingredients).toEqual(["1 lb spaghetti", "Salt for water"]);
        ...
        expect(recipe.components[1]?.steps[0]?.ingredients).toEqual(["4 oz guanciale, diced"]);
    ...
    test("should parse a simple recipe without metadata or named components", () => {
        ...
        expect(recipe.components[0]?.steps[0]?.text).toBe("Combine greens and dressing");
    ...
    test("should throw an error if content is missing", () => {
        const text = `= My Title`;
        expect(() => Recipe.parse(text)).toThrow("Recipe content is missing or invalid.");
    });

    test("should throw an error for an ingredient without a step", () => {
        const text = `= Bad Recipe\n- 1 cup flour`;
        expect(() => Recipe.parse(text)).toThrow("Ingredients must belong to a step.");
    });
});
...
describe("Recipe properties", () => {
    ...
    test("should return correct string properties", () => {
        ...
        expect(recipe.notes).toBe("A classic Roman pasta dish.\nUse guanciale for the most authentic flavor.");
    });
